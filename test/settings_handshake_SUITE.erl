...
times_out_on_no_ack_of_server_settings(Config) ->
    ...
    Bin = chatterbox_h2_frame_settings:send(#settings{}, ClientSettings),
    ...
    [{GoAwayH, GoAway}] = chatterbox_h2_frame:from_binary(GoAwayBin),
    ?assertEqual(?GOAWAY, (GoAwayH#frame_header.type)),
    ?assertEqual(?SETTINGS_TIMEOUT, (chatterbox_h2_frame_goaway:error_code(GoAway))),
    ok.
